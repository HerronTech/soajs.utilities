...
function addTenants(cb){
	...
	mongo.update("tenants",{"code":"JST2"},{$set:tenants[0]}, {upsert:true, multi:false, safe:true}, function(err1){
		if(err1){
			return cb(err1);
		}
		else {
			mongo.update("tenants", {"code": "JSTE"}, {$set: tenants[1]}, {upsert: true, multi: false, safe: true}, function (err2) {
				if (err2) {
					return cb(err2);
				}
				else {
